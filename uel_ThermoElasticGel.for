************************************************************************
!
! User element for transient fluid permeation, heat transfer, 
!  and large elastic deformation in 2D or 3D.  This is for plane 
!  strain, axisymetric, and 3D.
!
! Solution variables (or nodal variables) are the displacements, the
...
!
!  2D elements
!  *User Element,Nodes=4,Type=U?,Iproperties=2,Properties=14,Coordinates=2,Variables=?,Unsymm
!  1,2,11,12
!
!  3D elements
!  *User Element,Nodes=8,Type=U3,Iproperties=2,Properties=14,Coordinates=3,Variables=?,Unsymm
!  1,2,3,11,12
!
!
!     State Variables
...
!     Vmol   = props(3) ! Volume of a mole of fluid particles
!     chiL   = props(4) ! Chi parameter
!     chiH   = props(5) ! Chi parameter
!     thetaT = props(6) ! Tranition temperature
...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 Rt(nNode,1),Ktt(nNode,nNode),theta_tau,theta_t,dTdt,Cheat
      real*8 Ktherm,dTdX(2,1),heatGen,dHdT,AmatUT(3,1),SpUTMod(3,3)
      real*8 Kut(nDim*nNode,nNode),Ktu(nNode,nDim*nNode),AmatTU(2,4)
      real*8 Kct(nNode,nNode),Ktc(nNode,nNode),SpTUModFac(3,3),DmDtheta
      real*8 SpTUMod(3,3,3)
...
c         deallocate(globalSdv)
     ...
     +     ((coordsC(2,1)-coordsC(2,3))**two))
      ...
      do intpt=1,nIntPt
         ...
         dTdX = zero
         do k=1,nNode
            theta_tau = theta_tau + thetaNew(k)*sh(k)
            theta_t   = theta_t + thetaOld(k)*sh(k)
            do i=1,nDim
               dTdX(i,1) = dTdX(i,1) + thetaNew(k)*dshC(k,i)
            enddo
         enddo
         dTdt = (theta_tau - theta_t)/dtime
     ...
     +        F_tau,mu_tau,phi_t,theta_tau,
     +        T_tau,SpTanMod,
     +        phi_tau,dPdt,DphiDmu,DphidotDmu,
     +        Mfluid,DmDmu,DmDJ,Vmol,
     +        SpUCMod,SpCUModFac,
     +        Cheat,Ktherm,heatGen,dHdT,
     +        SpUTMod,SpTUModFac,DmDtheta)
         !
         !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
     ...
     +        )
         ...
         !
         !
         ResFac = Cheat*dTdt - heatGen
         !
         Rt = Rt + detmapJC*w(intpt)*
     +        (
     +        transpose(Nvec)*ResFac + Ktherm*matmul(dshC,dTdX)
     +        )
     ...
     +        )
         ...
         ! Compute/update the temperature tangent matrix
         !
         TanFac = -Cheat/dtime + dHdT
         !
         Ktt = Ktt + detmapJC*w(intPt)*
     +        (
     +        TanFac*matmul(transpose(Nvec),Nvec)
     +        - Ktherm*matmul(dshC,transpose(dshC))
     +        )
     ...
     +        )
         ...
         ! Compute/update the displacement - temperature tangent matrix.
         !  The F-bar method will have some effect, however we neglect that here.
         !
         AmatUT = zero
         AmatUT(1,1) = SpUTMod(1,1)
         AmatUT(2,1) = SpUTMod(2,2)
         AmatUT(3,1) = SpUTMod(1,2)
         !
         Kut = Kut + detMapJC*w(intpt)*
     +        (
     +        zero
c     +        matmul(matmul(transpose(Bmat),AmatUT),Nvec)
     +        )


         ! Compute/update the temperature - displacement tangent matrix.
         !  The F-bar method will have some effect, however we neglect that here.
         !
         SpTUMod = zero
         do i=1,nDim
            do k=1,nDim
               do l=1,nDim
                  SpTUMod(i,k,l) = SpTUMod(i,k,l)
     +                 + dTdX(k,1)*SpTUModFac(i,l)
               enddo
            enddo
         enddo
         !
         AmatTU = zero
         AmatTU(1,1) = SpTUMod(1,1,1)
         AmatTU(1,2) = SpTUMod(1,2,1)
         AmatTU(1,3) = SpTUMod(1,1,2)
         AmatTU(1,4) = SpTUMod(1,2,2)
         AmatTU(2,1) = SpTUMod(2,1,1)
         AmatTU(2,2) = SpTUMod(2,2,1)
         AmatTU(2,3) = SpTUMod(2,1,2)
         AmatTU(2,4) = SpTUMod(2,2,2)
         !
         Ktu = Ktu + detMapJC*w(intpt)*
     +        (
     +        zero
c     +        matmul(matmul(dshC,AmatTU),Gmat)
     +        )
         ...
         !
         Kct = Kct + detMapJC*w(intpt)*
     +        (
     +        DmDtheta*matmul(matmul(dshC,dTdX),Nvec)
     +        )


         ! Compute/update the temperature - chemical potential tangent matrix.
         !
         Ktc = Ktc + detMapJC*w(intpt)*
     +        (
     +        zero
     +        )
     ...
     +     Ru,Rc,Rt,
     +     Kuu,Kuc,Kut,Kcu,Kcc,Kct,Ktu,Ktc,Ktt,
     +     rhs,amatrx)
      !
...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 TanFac,Nvec(1,nNode),AmatUC(4,1),SpCUModFac(3,3),theta_tau
      real*8 Rt(nNode,1),Ktu(nNode,nDim*nNode),Kut(nDim*nNode,nNode)
      real*8 AmatCU(2,5),SpUCMod(3,3),Ktt(nNode,nNode),Kct(nNode,nNode)
      real*8 Ktc(nNode,nNode),theta_t,dTdt,dTdX(nDim,1),Cheat,Ktherm
      real*8 heatGen,dHdT,SpUTMod(3,3),SpTUModFac(3,3),SpTUMod(3,3,3)
      real*8 AmatUT(4,1),AmatTU(2,5),DmDtheta
...
c         deallocate(globalSdv)
      ...
      ! Obtain nodal displacements, chemical potentials,
      !  and temperatures
      !
      k = 0
      do i=1,nNode
     ...
     +        )
         ...
         AmatUT(2,1) = SpUTMod(2,2)
         AmatUT(3,1) = SpUTMod(1,2)
         AmatUT(4,1) = SpUTMod(3,3)
         !
         Kut = Kut + detMapJC*w(intpt)*AR*
     +        (
     +        zero
c     +        matmul(matmul(transpose(BmatAX),AmatUT),Nvec)
     +        )
     ...
     +                 + dTdX(k,1)*SpTUModFac(i,l)
         ...
         AmatTU(1,3) = SpTUMod(1,1,2)
         AmatTU(1,4) = SpTUMod(1,2,2)
         AmatTU(1,5) = SpTUMod(1,3,3)
         AmatTU(2,1) = SpTUMod(2,1,1)
         AmatTU(2,2) = SpTUMod(2,2,1)
         AmatTU(2,3) = SpTUMod(2,1,2)
         AmatTU(2,4) = SpTUMod(2,2,2)
         AmatTU(2,5) = SpTUMod(2,3,3)
         !
         Ktu = Ktu + detMapJC*w(intpt)*AR*
     +        (
     +        zero
c     +        matmul(matmul(dshC,AmatTU),GmatAX)
     +        )
...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 SpCUModFac(3,3),Rt(nNode,1),Ktu(nNode,nDim*nNode),theta_t
      real*8 Kut(nDim*nNode,nNode),Ktt(nNode,nNode),Ktc(nNode,nNode)
      real*8 Kct(nNode,nNode),theta_tau,dTdt,dTdX(nDim,1),Cheat,Ktherm
      real*8 heatGen,dHdT,SpUTMod(3,3),SpTUModFac(3,3),DmDtheta
      real*8 SpTUMod(3,3,3),AmatUT(6,1),AmatTU(3,9)
...
c         deallocate(globalSdv)
     ...
     +        )
         ...
         AmatUT(1,1) = SpUTMod(1,1)
         AmatUT(2,1) = SpUTMod(2,2)
         AmatUT(3,1) = SpUTMod(3,3)
         AmatUT(4,1) = SpUTMod(1,2)
         AmatUT(5,1) = SpUTMod(2,3)
         AmatUT(6,1) = SpUTMod(1,3)
         !
         Kut = Kut + detMapJC*w(intpt)*
     +        (
     +        zero
c     +        matmul(matmul(transpose(Bmat),AmatUT),Nvec)
     ...
     +                 + dTdX(k,1)*SpTUModFac(i,l)
         ...
         AmatTU(1,1) = SpTUMod(1,1,1)
         AmatTU(1,2) = SpTUMod(1,2,1)
         AmatTU(1,3) = SpTUMod(1,3,1)
         AmatTU(1,4) = SpTUMod(1,1,2)
         AmatTU(1,5) = SpTUMod(1,2,2)
         AmatTU(1,6) = SpTUMod(1,3,2)
         AmatTU(1,7) = SpTUMod(1,1,3)
         AmatTU(1,8) = SpTUMod(1,2,3)
         AmatTU(1,9) = SpTUMod(1,3,3)
         AmatTU(2,1) = SpTUMod(2,1,1)
         AmatTU(2,2) = SpTUMod(2,2,1)
         AmatTU(2,3) = SpTUMod(2,3,1)
         AmatTU(2,4) = SpTUMod(2,1,2)
         AmatTU(2,5) = SpTUMod(2,2,2)
         AmatTU(2,6) = SpTUMod(2,3,2)
         AmatTU(2,7) = SpTUMod(2,1,3)
         AmatTU(2,8) = SpTUMod(2,2,3)
         AmatTU(2,9) = SpTUMod(2,3,3)
         AmatTU(3,1) = SpTUMod(3,1,1)
         AmatTU(3,2) = SpTUMod(3,2,1)
         AmatTU(3,3) = SpTUMod(3,3,1)
         AmatTU(3,4) = SpTUMod(3,1,2)
         AmatTU(3,5) = SpTUMod(3,2,2)
         AmatTU(3,6) = SpTUMod(3,3,2)
         AmatTU(3,7) = SpTUMod(3,1,3)
         AmatTU(3,8) = SpTUMod(3,2,3)
         AmatTU(3,9) = SpTUMod(3,3,3)
         !
         Ktu = Ktu + detMapJC*w(intpt)*
     +        (
     +        zero
...
************************************************************************
     ...
     +     SpUTMod,SpTUModFac,DmDtheta)
      ...
      ! 12) specific heat, Cheat
      ! 13) thermal conductivity, Ktherm
      ! 14) heat source/sink therm, heatGen
      ! 15) derivative of heatGen with respect to temperature, dHdT
      ! 16) displacement - temperature modulus terms
      ! 17) temperature - displacement modulus terms
      ! 18) derivative of permeability with temperature, DmDtheta
      ...
      real*8 SpCUModFac(3,3),detFs,chiL,chiH,thetaT,Delta,Cheat,Ktherm
      real*8 heatGen,dHdT,SpUTMod(3,3),SpTUModFac(3,3),DmDtheta
     ...
     +     half=1.d0/2.d0)
      ...
      !
      Gshear = Nr*Rgas*theta
      !
      ! Compute the chi parameter
      !
      chi = half*(chiL + chiH)
     +     - half*(chiL - chiH)*dtanh((theta-thetaT)/Delta)
     ...
     +                       (dTRdF(i,m,k,n)*F_tau(j,m)*F_tau(l,n))/detF
      ...
      ! Compute the temperature - displacement modulus
      !
      SpTUModFac = Ktherm*Iden


      ! Compute the displacement - temperature modulus
      !
      SpUTMod = (Nr*Rgas)*(B_tau - Iden)/detF
...
************************************************************************
     ...
     +     rhs,amatrx)
      ...
      real*8 Kcu(nNode,nDim*nNode),amatrx(ndofel,ndofel),Rt(nNode,1)
      real*8 Ktt(nNode,nNode),Kct(nNode,nNode),Ktc(nNode,nNode)
      real*8 Kut(nDim*nNode,nNode),Ktu(nNode,nDim*nNode)
